...
class UIComponents {
    ...
    async generateAndShowSnakeFact(snakeName, modalDescription, levelData) {
        ...
        const loadingMessage = window.snakeFacts.getLoadingMessage(snakeName, levelData);
        modalDescription.innerHTML = loadingMessage;
        
        try {
            const formattedSnakeFact = await window.snakeFacts.generateAndFormatSnakeFact(snakeName, levelData);
            modalDescription.innerHTML = formattedSnakeFact;
        } catch (error) {
